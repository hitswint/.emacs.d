...
;; Copyright (C) 1994, 2013 Joseph W. Reiss
...
;; 02-Oct-2013|1.08|http://joereiss.net/misc/cycle-mini.el|
...
;;
;; v1.08
;;  - Eliminated use of deprecated 'make-local-hook' function
;;  - Updated to leverage the 'minibuffer-completing-file-name'
;;    variable
;; v1.07
...
;; Not really a bug, but an ugly kludge.  There's no really good,
;; reliable way to determine when we're being asked to complete a
;; buffer name.  But we need to know that in order to alter our
;; sorting behavior based on the 'cycle-mini-sort-buffers'
;; variable.  So I've finally been forced to fall back on examining
;; the minibuffer prompt for the word "buffer". This is ugly, but
  ...
  (setq cycle-mini-completion-type
        (cond
         (minibuffer-completing-file-name
          'file)
         ((string-match-p "[Bb]uffer" (minibuffer-prompt))
          'buffer)
         (t
          'other)))
  (cycle-mini-no-completion)
