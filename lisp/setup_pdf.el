...
(use-package pdfgrep
  ...
  (defun pdfgrep-default (&optional arg keys)
    (interactive "P")
    (let* ((string-to-grep (read-string "Default pdfgrep: "))
           (string-to-escape "\\( \\|(\\|)\\|\\[\\|\\]\\|{\\|}\\|'\\|&\\)")
           (default-directory (helm-current-directory))
           (pdf-files (cond (keys
                             (remove nil (cl-loop for key in keys collect (car (bibtex-completion-find-pdf key)))))
  ...
  (defun pdfgrep-opened (&optional arg)
    ...
    (let* ((string-to-escape "\\( \\|(\\|)\\|\\[\\|\\]\\|{\\|}\\|'\\|&\\)")
           ...
           (pdf-files (append (cl-loop for buffer in (buffer-list)
                              ...
                              (cl-loop for x in (split-string (shell-command-to-string "pgrep -x llpp") "\n" t)
                                       collect (shell-command-to-string
      ...
      (cl-flet ((escape-local (x)
        ...
        (if pdf-files
            (let ((string-to-grep (read-string "Opened pdfgrep: ")))
              (if (or arg (= (length pdf-files) 1))
                  (pdfgrep (concat (pdfgrep-default-command) (escape-local string-to-grep)
                                   (cl-loop for x in pdf-files concat (concat " " (escape-local x)))))
                (rg-run string-to-grep "pdf" (expand-file-name zotero-storage) nil nil (mapcar #'escape-local pdf-files))))
