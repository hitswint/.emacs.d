...
(use-package dired
  ...
  (defvar file-extension-app-alist (append '(("pdf" . "pdfviewer.sh") ("djvu" . "llpp") ("epub" . "llpp") ("chm" . "xchm")
                                             ...
                                             ("wps" . "wps") ("doc" . "wps") ("docx" . "wps")
                                             ("et" . "et") ("xls" . "et") ("xlsx" . "et") ("csv" . "et")
                                             ("dps" . "wpp") ("ppt" . "wpp") ("pptx" . "wpp")
...
(use-package image-dired
  ...
  (add-hook 'dired-mode-hook (lambda ()
                               (bind-key "M-I" #'(lambda () (interactive) (if (not (global-image-dired-minor-mode 'toggle))
                                                                              (when-let ((buf (get-buffer image-dired-thumbnail-buffer)))
                                                                                (swint-kill-buffer buf))
                                                                            (delete-other-windows)
                                                                            (image-dired-display-thumbs-all)
                                                                            (when-let ((win (get-buffer-window image-dired-thumbnail-buffer)))
                                                                              (select-window win))))
                                         dired-mode-map)))
  :config
  ...
  (add-hook 'image-dired-thumbnail-mode-hook (lambda ()
                                               ...
                                               (visible-mark-mode -1)))
  (bind-key "SPC" #'(lambda () (interactive) (let ((image-file-name (buffer-file-name (window-buffer (image-dired-display-window)))))
                                               (if (equal image-file-name (image-dired-original-file-name))
                                                   (image-dired-display-next 1)
                                                 (image-dired-display-this))))
            image-dired-thumbnail-mode-map)
  (bind-key "S-SPC" #'(lambda () (interactive) (let ((image-file-name (buffer-file-name (window-buffer (image-dired-display-window)))))
                                                 (if (equal image-file-name (image-dired-original-file-name))
                                                     (image-dired-display-previous 1)
                                                   (image-dired-display-this))))
            image-dired-thumbnail-mode-map)
  (bind-key "C-j" 'image-dired-thumbnail-display-external image-dired-thumbnail-mode-map)
  (bind-key "RET" #'(lambda () (interactive)
                      (when-let ((orig-file (image-dired-original-file-name))) (eaf-find-file-other-window orig-file)))
            image-dired-thumbnail-mode-map)
  (bind-key "C-a" 'image-dired-move-beginning-of-line image-dired-thumbnail-mode-map)
  (bind-key "M-I" #'(lambda () (interactive) (global-image-dired-minor-mode -1) (swint-kill-buffer)) image-dired-thumbnail-mode-map)
  (bind-key "q" #'(lambda () (interactive) (global-image-dired-minor-mode -1) (swint-kill-buffer)) image-dired-thumbnail-mode-map)
  (bind-key "a" 'image-dired-move-beginning-of-line image-dired-thumbnail-mode-map)
  (bind-key "e" 'image-dired-move-end-of-line image-dired-thumbnail-mode-map)
  ...
  (cl-loop for key in '("c" "C" "R" "D" "y" "v" "M-w" "M-W" "M-g o" "!" "C-o" "o")
           do (lexical-let ((key key))
  ...
  (advice-add 'image-dired-jump-original-dired-buffer :around #'(lambda (fn)
                                                                  (image-dired-track-original-file)
                                                                  (funcall fn)
                                                                  (let ((dired-buf (image-dired-associated-dired-buffer)))
                                                                    (unless (eq (window-buffer) dired-buf)
                                                                      (switch-to-buffer-other-window dired-buf)))))
  (advice-add 'image-dired-jump-thumbnail-buffer :around #'(lambda (fn)
                                                             (image-dired-track-thumbnail)
                                                             (funcall fn)
                                                             (unless (eq (window-buffer) (get-buffer image-dired-thumbnail-buffer))
                                                               (switch-to-buffer-other-window image-dired-thumbnail-buffer)))))
;; ===============image-dired==================
;;; peep-dired
;; ================peep-dired==================
...
(use-package dired-subtree
  ...
  (defun dired-subtree-remove-all ()
    (interactive)
    (if (dired-subtree-in-subtree-p)
        (progn (while (dired-subtree-in-subtree-p) (dired-subtree-remove))
