...
;;;; keybindings
  ...
  (bind-key "C-x M-f" #'(lambda (&optional arg) (interactive "P") (let ((host (helm-select-host))
  ...
  (bind-key "C-x M-F" #'(lambda (&optional arg) (interactive "P") (let ((tramp-completion-use-auth-sources nil)
                                                                        (host (helm-select-host))
                                                                        (method (if arg "ssh" "rpc")))
                                                                    (helm-find-files-1 (format "/%s:%s:%s/" method host (expand-file-name (getenv "HOME")))))))
  (bind-key "C-x C-M-f" #'(lambda () (interactive) (let ((host (helm-select-host))
                                                         (curr (or buffer-file-name (helm-current-directory))))
                                                     (find-file (concat "/mnt/sshfs/" host (expand-file-name curr))))))
  (bind-key "C-x C-M-S-f" #'(lambda () (interactive) (let ((host (helm-select-host)))
                                                       (helm-find-files-1 (concat "/mnt/sshfs/" host (expand-file-name (getenv "HOME")) "/")))))
    ...
    (let ((default-directory (or dir (helm-current-directory))))
      (if (or current-prefix-arg helm-current-prefix-arg)
          (let ((helm-ag-command-option (concat helm-ag-command-option " -u"
                                                (let ((lines (read-string "Print M/N lines before/after matches(default: 2,3): ")))
                                                  (unless (string-empty-p lines)
                                                    (let ((lines-list (split-string lines "," nil "[ \t\n]+")))
                                                      (if (= (length lines-list) 1)
                                                          (format " --context=%s" (car lines-list))
                                                        (let ((before (car lines-list))
                                                              (after (cadr lines-list)))
                                                          (concat (unless (string-empty-p before)
                                                                    (format " --before=%s" before))
