...
;;;; helm-related-to-persp
    ...
    (let ((buffer (get-buffer BUFFER-OR-NAME)))
      (if (bound-and-true-p persp-mode)
          (cl-loop for persp in (nreverse (cons  "i" (nreverse (delete "i" (persp-names)))))
                   when (or (memq buffer (persp-buffers (gethash persp (frame-parameter nil 'perspectives-hash))))
                            (string-equal persp "i"))
                   do (if (memq buffer (persp-buffers (frame-parameter nil 'persp-curr)))
  ...
  (defun swint-helm-switch-persp/other-window (buffer)
    ...
    (if (or (not (bound-and-true-p persp-mode))
            (memq buffer (persp-buffers (frame-parameter nil 'persp-curr))))
