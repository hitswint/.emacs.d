...
(use-package avy
  ...
  (setq avy-keys-alist '((avy-goto-line . ,(append (number-sequence ?a ?z) (number-sequence ?A ?Z)))
                         (avy-copy-region . ,(append (number-sequence ?a ?z) (number-sequence ?A ?Z)))
                         (avy-copy-line . ,(append (number-sequence ?a ?z) (number-sequence ?A ?Z)))
                         (avy-move-region . ,(append (number-sequence ?a ?z) (number-sequence ?A ?Z)))
                         (avy-move-line . ,(append (number-sequence ?a ?z) (number-sequence ?A ?Z)))))
  ;; at-full会造成ace-pinyin显示不正确。
  (setq avy-styles-alist '((avy-goto-char . at)
                           (avy-goto-char-2 . at)
                           (avy-goto-word-0 . at)
                           (avy-goto-word-1 . at)
                           (swint-avy-goto-char-timer . at)))
