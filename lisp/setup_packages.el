...
(use-package multiple-cursors
  ...
  (add-to-list 'mc--default-cmds-to-run-once 'mc/kill-ring-save-combined)
  (defun mc/calc-sum ()
    (interactive)
  ...
  (defun mc/kill-ring-save-combined ()
    (interactive)
    (kill-new (message "%s" (mapconcat 'identity (mc--ordered-region-strings) "\n"))))
  ...
  (define-key mc/keymap (kbd "M-W") 'mc/kill-ring-save-combined)
...
(use-package vlf
  ...
  (bind-key "C-s" 'isearch-forward vlf-prefix-map)
  (bind-key "C-r" 'isearch-backward vlf-prefix-map)
  (bind-key "M-g M-g" 'swint-vlf-goto-line vlf-prefix-map)
  (bind-key "G" 'swint-vlf-revert vlf-mode-map)
  ...
  (defun swint-vlf-revert ()
    (interactive)
    (let ((filename (buffer-file-name)))
      (kill-buffer)
      (vlf filename)))
  (defun swint-vlf-isearch-wrap ()
    (let* ((rg-result (shell-command-to-string (format "rg -nb %s\"%s\" \"%s\""
                                                       (if (or isearch-regexp pinyin-search-activated) "" "-F ")
                                                       (if pinyin-search-activated
                                                           (pinyin-search--pinyin-to-regexp isearch-string)
                                                         isearch-string)
                                                       (buffer-file-name))))
           (ln-alist (cl-loop for x in (split-string rg-result "\n" t)
                              for x-list = (split-string x ":")
                              collect (cons (string-to-number (cl-first x-list))
                                            (string-to-number (cl-second x-list)))))
           (target-entry (when ln-alist
                           (or (let ((compare-op (if isearch-forward #'> #'<)))
                                 (cl-find-if (lambda (x) (funcall compare-op (cdr x)
                                                                  (if isearch-forward vlf-end-pos vlf-start-pos)))
                                             (if isearch-forward ln-alist (reverse ln-alist))))
                               (cl-first (if isearch-forward ln-alist (last ln-alist))))))
           vlf-tune-enabled)
      (when target-entry
        (let ((target-byte (cdr target-entry)))
          (when (> target-byte vlf-file-size)
            (swint-vlf-revert))
          (vlf-jump-to-chunk (1+ (/ target-byte vlf-batch-size)))
          (goto-char (1+ (byte-to-position (- target-byte vlf-start-pos))))
          (goto-char (funcall (intern (format "line-%s-position" (if isearch-forward 'beginning 'end)))))))))
  ...
  (defun swint-vlf-end-of-file ()
    ...
    (when (pos-visible-in-window-p (point-max))
      (when (and (equal vlf-end-pos vlf-file-size)
                 (> (string-to-number (shell-command-to-string (format "wc -c %s" (buffer-file-name))))
                    vlf-file-size))
        (swint-vlf-revert))
      (vlf-end-of-file))
    (end-of-buffer))
  (defun swint-vlf-line-to-byte (n filename)
    (string-to-number
     (if (< (time-to-seconds (time-subtract (current-time) (file-attribute-modification-time (file-attributes filename)))) 5)
         (shell-command-to-string (format "head -n %s %s | wc -c" n filename))
       (shell-command-to-string (format "n=%s; f=\"%s\"; m=$(wc -l < \"$f\"); ((2*n<m)) && head -n $n \"$f\" | wc -c || echo $(( $(wc -c < \"$f\")-$(tail -n $((m-n)) \"$f\" | wc -c)))" n filename)))))
  (defun swint-vlf-byte-to-line (b filename)
    (string-to-number
     (if (< (time-to-seconds (time-subtract (current-time) (file-attribute-modification-time (file-attributes filename)))) 5)
         (shell-command-to-string (format "head -c %s %s | wc -l" b filename))
       (shell-command-to-string (format "n=%s; f=\"%s\"; m=$(wc -c < \"$f\"); ((2*n<m)) && head -c $n \"$f\" | wc -l || echo $(( $(wc -l < \"$f\") - $(tail -c $((m-n)) \"$f\" | wc -l) ))" b filename)))))
  (defun swint-vlf-goto-line (n &optional byte)
    (interactive (if (vlf-no-modifications)
                     (list (read-number "Go to line: "))))
    (let ((line-byte (or byte (swint-vlf-line-to-byte (1- n) (buffer-file-name))))
          vlf-tune-enabled)
      (when (> line-byte 0)
        (when (> line-byte vlf-file-size)
          (swint-vlf-revert))
        (vlf-jump-to-chunk (1+ (/ line-byte vlf-batch-size)))
        ;; (let ((batch-start-line (swint-vlf-byte-to-line vlf-start-pos filename)))
        ;;   (goto-line (- n batch-start-line)))
        (goto-char (1+ (byte-to-position (- line-byte vlf-start-pos))))))))
;; =======================vlf======================
;;; easy-kill
;; =====================easy-kill==================
  ...
  (defun rg-copy-things ()
    ...
    (let ((file-name (rg-find-filename)))
      (if (file-in-directory-p file-name "~/Zotero")
          (let* ((input-char (read-char "char [ektd]: "))
                 (entry (bibtex-completion-get-entry-for-pdf file-name))
                 (new (cond ((= input-char ?e)
                             (with-temp-buffer
                               (let ((key (bibtex-completion-get-value "=key=" entry)))
                                 (if (not (bound-and-true-p ebib--databases))
                                     (message "ebib needs to be opened.")
                                   (ebib--format-entry key (ebib--find-db-for-key key ebib--cur-db))
                                   (buffer-substring-no-properties (point-min) (point-max))))))
                            ((= input-char ?k)
                             (bibtex-completion-get-value "=key=" entry))
                            ((= input-char ?t)
                             (ebib-unbrace (bibtex-completion-get-value "title" entry)))
                            ((= input-char ?d)
                             (ebib-unbrace (bibtex-completion-get-value "doi" entry))))))
            (if new
                (kill-new (message "%s" new))
...
(use-package pixel-scroll
  ...
  (defun pixel-scroll-window-move-up (&optional arg)
    ...
    (if (display-graphic-p)
        (pixel-scroll-precision-scroll-down (* 40 (or arg 1)))
      (call-interactively 'window-move-up)))
  (defun pixel-scroll-window-move-down (&optional arg)
    (interactive "p")
    (if (display-graphic-p)
        (pixel-scroll-precision-scroll-up (* 40 (or arg 1)))
