...
(use-package vlf
  ...
  (defun swint-vlf-isearch-wrap ()
    ...
    (let* ((filename (buffer-file-name))
           ...
           (compare-op (if isearch-forward #'> #'<))
           (target-line (when ln-list
                          (or (cl-find-if (lambda (x) (funcall compare-op x batch-line))
                                          (if isearch-forward ln-list (reverse ln-list)))
                              (cl-first (if isearch-forward ln-list (last ln-list))))))
           vlf-tune-enabled)
      (when target-line
        (vlf-jump-to-chunk (1+ (/ (swint-vlf-line-to-byte target-line filename) vlf-batch-size)))
        (goto-char (funcall (intern (format "point-%s" (if isearch-forward 'min 'max))))))))
  ...
  (defun swint-vlf-line-to-byte (n filename)
    (string-to-number  ;; (shell-command-to-string (format "head -n %s %s | wc -c" n filename))
  ...
  (defun swint-vlf-byte-to-line (b filename)
    (string-to-number ;; (shell-command-to-string (format "head -c %s %s | wc -l" vlf-start-pos filename))
     (shell-command-to-string (format "n=%s; f=\"%s\"; m=$(wc -c < \"$f\"); ((2*n<m)) && head -c $n \"$f\" | wc -l || echo $(( $(wc -l < \"$f\") - $(tail -c $((m-n)) \"$f\" | wc -l) ))" b filename))))
  (defun swint-vlf-goto-line (n)
    (interactive (if (vlf-no-modifications)
                     (list (read-number "Go to line: "))))
    (vlf-revert nil t)
    (let* ((filename (buffer-file-name))
           (batch-num (1+ (/ (swint-vlf-line-to-byte n filename) vlf-batch-size)))
           vlf-tune-enabled)
      (vlf-jump-to-chunk batch-num)
