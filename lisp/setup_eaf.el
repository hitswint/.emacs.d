...
(use-package eaf
  ...
  (use-package eaf-pdf-viewer
    ...
    (defun eaf-pdf-annot-list ()
      (interactive)
      (let* ((annots-real (mapcar #'(lambda (x) (gethash "fitz-A0" (json-parse-string x)))
                                  (reverse (hash-table-values (json-parse-string (eaf-pdf-get-document-annots))))))
             (annots-display (mapcar #'(lambda (x)
                                         (let ((content-field (gethash "content" (gethash "info" x))))
                                           (if (string-empty-p content-field)
                                               (concat "type_name: " (gethash "type_name" x))
                                             (concat "content: " (replace-regexp-in-string "\n" "" content-field)))))
                                     annots-real))
             (selected-annot (helm-comp-read "Select annots: " (-zip-pair annots-display annots-real)
                                             :buffer "*helm eaf annots-swint*")))
        (eaf-pdf-jump-to-annot selected-annot)))
    ...
    (eaf-bind-key eaf-pdf-annot-list "M-a a" eaf-pdf-viewer-keybinding)
