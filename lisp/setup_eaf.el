...
(use-package eaf
  ...
  (defun eaf-translate-text (text)
    (let* ((trimmed-text (replace-regexp-in-string "\n" " " (string-trim text)))
           (sdcv-result (sdcv-search-with-dictionary trimmed-text sdcv-dictionary-list t)))
      (if (not (string-match-p  "\\`[ \t\n\r]*\\'" sdcv-result))
          (swint-display-dict "*sdcv*" sdcv-result 'sdcv-output-cleaner)
        (bing-dict-brief-cb-at-point text t)
        (when (equal (current-message) "No results")
          (ellama-translate-at-point text)))))
  ...
  (use-package eaf-pdf-viewer
    ...
    (defun eaf-pdf-copy-things ()
      (interactive)
      (if (file-in-directory-p eaf--buffer-url "~/Zotero")
          (let* ((input-char (read-char "char [ektd]: "))
                 (entry (bibtex-completion-get-entry-for-pdf eaf--buffer-url))
    ...
    (eaf-bind-key eaf-pdf-search-text-forward "C-s" eaf-pdf-viewer-keybinding)
    (eaf-bind-key eaf-pdf-search-text-backward "C-r" eaf-pdf-viewer-keybinding)
    ...
    (eaf-bind-key eaf-pdf-copy-things "C" eaf-pdf-viewer-keybinding)
    ...
    (advice-add 'eaf--monitor-emacs-kill :before #'(lambda () (ignore-errors
                                                                (let* ((pdf-urls "")
                                                                       (pdf-opened-files (dolist (buffer (eaf--get-eaf-buffers) pdf-urls)
                                                                  ...
                                                                  (unless (string-empty-p pdf-opened-files)
                                                                    (write-region pdf-opened-files nil pdf-opened-file-path))))))
                                                                      ...
                                                                      (progn (let ((pdf-opened-files (with-temp-buffer (insert-file-contents pdf-opened-file-path)
                                                                               ...
                                                                               (cl-loop for pdf in (cdr pdf-opened-files)
                                                                                        ...
                                                                                        do (progn
                                                                                             (sit-for 0.2)
                                                                                             (eaf-open pdf))))
                                                                             (delete-file pdf-opened-file-path))
                                                                    (eaf-pdf-delete-invalid-file-record-from-history)
                                                                    (funcall fn)))))
