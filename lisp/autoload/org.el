...
(defun swint-org-open-at-point (&optional in-emacs)
  ...
  (if (and (org-in-regexp org-link-bracket-re 1)
    ...
    (let ((annotate-file (when (or (ignore-errors (org-at-top-heading-p))
                                   ;; (org-at-keyword-p)
                                   (org-at-property-p))
                           (org-get-annotate-file)))
          (annotate-page (unless (or (org-in-regexp org-link-any-re)
                                     (org-element-type-p (org-element-context) '(citation citation-reference)))
                           (org-get-annotate-page)))
...
(defun swint-annotate-generic-new ()
  ...
  (let* ((annotate-file (org-get-annotate-file))
         ...
         (eaf-pdf-page (when-let ((annotate-buffer (and annotate-file
                                                        (eaf-interleave--find-buffer (expand-file-name annotate-file)))))
