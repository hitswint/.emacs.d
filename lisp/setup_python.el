...
(use-package pyvenv
  ...
  (defvar swint-python-plot-hash (let ((colors (nconc (list "r" "g" "b" "y" "c" "m" "k" "C0" "C1" "C2" "C3" "C4" "C5" "C6" "C7" "C8" "C9")
                                   ...
                                   (ht ("data" "")
                                       ...
                                       ("colors" colors)
                                       ("markercolors" colors)  ;空字符串对应空心标记
                                       ("lines" (list "-" "--" "-." ":"))
                                   ...
                                   collect (cond
                                            ((equal x "text")
                                             (concat (format "text='%s'" (read-string "text($\\\\alpha$): "))
                                                     ...
                                                     (format ",text_config_optional='%s'" (read-string "text_config_optional: " "12,s,k"))))
                                            ((equal x "annotate")
                                             (concat (format "annotate='%s'" (read-string "annotate($\\\\alpha$): "))
                                                     ...
                                                     (format ",annotate_config_optional='%s'" (read-string "annotate_config_optional: "
                                                                                                           "12,s,k,arrowstyle=\"simple\",connectionstyle=\"arc3\",edgecolor=\"k\",facecolor=\"w\",fill=False,linestyle=\"-\",linewidth=2"))))
                                            ((equal x "axline")
    ...
    (let (data-string files-list files-string columns-string file-x-string column-x-string style-string shell-command-args send-string-args)
      (cl-flet ((plot-file-setup (data-file &optional is-x-p)
        ...
        (let* ((config-list (helm-comp-read "Configs: " (list "None" "rows" "labels" "fonts" "sizes" "colors" "markercolors" "lines" "markers" "hatchs" "polyfit" "twinx" "xlog" "ylog" "save" "half_width" "animate")
                                            :marked-candidates t
                                            :buffer "*helm python fig config-swint*"))
               ;; 从列表中除去多个元素，也可以用：(cl-set-difference config-list '("twinx" "xlog" "ylog" "save" "half_width" "animate" "None") :test 'equal)
               (args-alist (cl-loop for x in (seq-difference config-list '("twinx" "xlog" "ylog" "save" "half_width" "animate" "None"))
          ...
          (setq shell-command-args (concat (mapconcat #'(lambda (x)
                                           ...
                                           (if (member "half_width" config-list) " --half_width ")
          ...
          (setq send-string-args (concat (if args-alist
                                         ...
                                         (if (member "half_width" config-list) ",half_width=True")
...
(use-package jupyter
  ...
  (defun swint-jupyter-run-repl ()
    ...
    (if (and (eq major-mode 'org-mode)
             (org-babel-jupyter--src-block-kernel-language))
        (revert-buffer nil t)
