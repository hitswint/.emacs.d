...
;;; 处理过长的文件名
(defun make-auto-save-file-name/around (fn)
  (let* ((orig-name (funcall fn))
         (filename (file-name-nondirectory orig-name)))
    (if (> (string-bytes filename) 250)
        (concat (file-name-directory orig-name)
                (if (multibyte-string-p filename)
                    (substring (string-as-multibyte (s-left 250 (string-as-unibyte filename))) 0 -2)
                  (s-left 250 filename))
                "#")
      orig-name)))
(advice-add 'make-auto-save-file-name :around #'make-auto-save-file-name/around)
(let ((day (* 60 60 24))
      (week (* 60 60 24 7))
      (month (* 60 60 24 30))
